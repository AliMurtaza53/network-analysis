...
#      raise utils.NotYetAttemptedException
   ...
   def FrankWolfeStepSize(self, targetFlows, precision = FRANK_WOLFE_STEPSIZE_PRECISION):
      ...
      # If derivative has same sign at both endpoints, choose the better endpoint
      if val_a == 0:
         return a
      if val_b == 0:
         return b
      if val_a * val_b > 0:
         # No sign change: pick 0 if derivative positive (ascending), else 1 if negative (descending)
         return a if val_a > 0 else b

      # Bisection to find root where the derivative changes sign
   ...
   def userEquilibrium(self, stepSizeRule = 'MSA',  
      ...
      while iteration < maxIterations:
         ...
         if stepSizeRule == 'FW':
            ...
            # Fallback to a diminishing step if FW returns a near-zero step (prevents stalling)
            if stepSize <= 1e-10:
   ...
   def findLeastEnteringLinks(self):
      """
      Return the node ID with the minimum number of entering links.
