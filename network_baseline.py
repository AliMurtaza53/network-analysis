...
#      raise utils.NotYetAttemptedException
   ...
   def userEquilibrium(self, stepSizeRule = 'MSA',  
      ...
      while iteration < maxIterations:
         ...
         relative_gaps.append(gap)         # Record gap value for the current iteration 
...
def run_demo():
   ...
   for name, net_path, trips_path in samples:
      ...
      with cProfile.Profile() as profile:
         net.userEquilibrium(step_rule, max_itrn, target_rgap, net.averageExcessCost)
      ...
      final_gap = relative_gaps[-1] if 'relative_gaps' in globals() and len(relative_gaps) > 0 else net.averageExcessCost()
      total_time = t1 - t0
      print(f"{name}: iterations={len(relative_gaps) if 'relative_gaps' in globals() else 'n/a'}, final_gap={final_gap:.6g}, total_time_s={total_time:.3f}, flows={flows_path}")
