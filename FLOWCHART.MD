...
flowchart TD
    ...
    subgraph S1[Quick Validation - validate.py]
        B0 --> B1[Parse CLI args: net-file, trips-file, --skip-tests, --runs]
        B1 --> B2{Skip unit tests?}
        B2 -- "No" --> B3[Run unit tests via tests.py / grader.py]
        B2 -- "Yes" --> B4[Construct Network from TNTP<br/>Network(net_file, trips_file)]
        B3 --> B4

        B4 --> B5[Run net.userEquilibrium()<br/>(FW or MSA)]
        B5 --> B6[Collect convergence data<br/>relative_gaps, iteration_times]
        B6 --> B7[Export flows & metrics<br/>to outputs/ files]
    end
    ...
    subgraph S2[Core UE Solve - network.py]
        ...
        C2 --> C3[Call userEquilibrium(stepSizeRule,<br/>maxIterations, targetGap, gapFunction, stepType)]
        ...
        C7 --> C8{Converged?<br/>gap ≤ targetGap or maxIterations}
        C8 -- "No" --> C3
    ...
    %% ========= POLICY ANALYSIS =========
    subgraph S3[Policy Analysis - policies & experiments]
        ...
        D3 --> D4[Compute metrics via get_metrics()<br/>TSTT, relative_gap]
        D4 --> D5[Compare baseline vs policy<br/>before/after metrics]
    end

    %% ========= ADVANCED TESTING =========
    subgraph S4[Advanced Testing & Benchmarking - tests]
        E0 --> E1[Select protocol files<br/>tests/protocol/*.txt]
        ...
        E2 --> E4[Loop over runs:<br/>load Network, run UE / metric eval]
        E3 --> E4

        E4 --> E5[Aggregate or log stats<br/>CSV/JSON, optional cProfile]
        E5 --> E6[Optional comparisons:<br/>compare_networks.py, compare_results.py]
    end
